...
fn part1(input: &str) -> usize {
    let (workflows, machine_parts) = parse(input);

    machine_parts
        .iter()
        .filter(|part| is_accepted(part, &workflows))
        .map(|part| part.sum_of_rating())
        ...
        for rule in &current_workflow.rules {
            if rule.rule_kind == RuleType::RatingCheck {
                let specific_rating = match rule.which_rating {
                    ...
                    Some(RatingType::A) => part.a,
                    Some(RatingType::S) => part.s,
                    None => {
                        panic!(
                            "We should have a which_rating when rule_type == RuleType::RatingCheck"
                        )
                    }
                };

                let passes_check = match rule.symbol {
                    Some(SymbolType::GreaterThan) => {
                        specific_rating > rule.rate_check_amount.unwrap()
                    }
                    Some(SymbolType::LesserThan) => {
                        specific_rating < rule.rate_check_amount.unwrap()
                    }
                    None => {
                        panic!("We should have a symbol when rule_type == RuleType::RatingCheck")
                    }
                };

                // If the part doesn't pass this rating check, we should continue on to the next
                // rule.
                if !passes_check {
                    continue;
                }
            ...
            if let OutcomeType::SendToWorkflow(next) = &rule.outcome_type {
                ...
                return false;
            } else if rule.outcome_type == OutcomeType::AcceptImmediately {
                return true;
            }
        }
    }
...
fn parse<'a>(input: &'a str) -> (Vec<Workflow>, Vec<MachinePart>) {
            ...
            let rules: Vec<Rule> = rules
                ...
                .map(|raw_rule| {
                    if raw_rule.contains(":") {
                        let (_, [which_rating, symbol, rate_check_amount, outcome_type]) =
                        ...
                        return Rule {
                            ...
                            which_rating: match which_rating {
                                ...
                                "a" => Some(RatingType::A),
                                "s" => Some(RatingType::S),
                                other => panic!("Cannot parse '{other}' into a valid RatingType."),
                            },
                            symbol: match symbol {
                                ">" => Some(SymbolType::GreaterThan),
                                "<" => Some(SymbolType::LesserThan),
                                other => panic!("Cannot parse '{other}' into a valid SymbolType."),
                            },
                            rate_check_amount: Some(rate_check_amount.parse().unwrap()),
                        };
                    } else {
                        let (_, [catch_all]) =
    ...
    let ratings_regex = Regex::new(r"\{x=(\d+),m=(\d+),a=(\d+),s=(\d+)\}").unwrap();

    let machine_parts: Vec<MachinePart> = split[1]
        .split("\n")
        .map(|line| {
            let (_, [x, m, a, s]) = ratings_regex.captures(line).unwrap().extract();
            MachinePart {
                x: x.parse().unwrap(),
                m: m.parse().unwrap(),
                a: a.parse().unwrap(),
